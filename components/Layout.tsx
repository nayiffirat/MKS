...
import { Home, Users, BookOpen, FileText, ClipboardList, Menu, X, LogOut, Settings, ChevronRight, User, Bell, Phone, UserCircle, Plus, PieChart, Sprout, CalendarCheck, ChevronLeft, Package, Truck, CreditCard, Receipt, Wallet, History as HistoryIcon, Printer, Shield, Clock, MessageCircle, Trash2, TrendingDown, Map, Sparkles, LineChart } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onNavigate }) => {
  ...
  const navGroups = [
    { title: "Zeka & Asistan", items: [{ id: 'AI_ASSISTANT', icon: Sparkles, label: 'Zirai Asistan (Gemini AI)' }, { id: 'SMART_STOCK', icon: LineChart, label: 'Akıllı Stok Tahmini' }] },
                    ...
                    <div className="space-y-0.5">
                        {group.items.map(item => (
                             <button key={item.id} onClick={() => handleNavClick(item.id as ViewState)} className={`w-full flex items-center justify-between px-3 py-2 rounded-xl transition-all duration-300 ${currentView === item.id ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'text-stone-400 hover:bg-stone-800'}`}>
                                 ...
                                 {(item as any).badge !== undefined && (item as any).badge > 0 && <span className="bg-red-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full">{(item as any).badge}</span>}
                             </button>
                        ))}
                    </div>
                </div>
